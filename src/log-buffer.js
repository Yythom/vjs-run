...
// 超过 MAX 才截到 KEEP：留出余量，避免到达上限后每来一个 chunk 都 slice 拷贝一整段
const MAX_LOG_LENGTH = 200_000; // 单个项目缓存字符上限，防止内存泄露
const KEEP_LOG_LENGTH = 150_000; // 截断时保留尾部的长度
...
export function appendLog(projectId, chunk) {
  ...
  if (next.length > MAX_LOG_LENGTH) {
    // 保留最新的日志段
    logBuffers.set(projectId, next.slice(next.length - KEEP_LOG_LENGTH));
  } else {
