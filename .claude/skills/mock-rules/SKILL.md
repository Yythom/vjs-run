...
**先解析出 `<base>`，别猜端口。** host/port 在 config.json 里，缺省 `127.0.0.1:3002`：

```bash
CFG=~/Library/Application\ Support/vjtools/config.json
BASE="http://$(node -e '
  const fs=require("fs");
  let c={}; try{c=JSON.parse(fs.readFileSync(process.argv[1],"utf8"))}catch(_){}
  process.stdout.write(`${c.mockHost||"127.0.0.1"}:${c.mockPort||3002}`);
' "$CFG")"
```
