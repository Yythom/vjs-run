...
这个路径 app 内不可配置（config.json 里的 `mockRulesFile` 会被 normalize 重置，改了也不生效）。
CLI 会自动定位，通常无需关心。（e2e/隔离环境用 `VJTOOLS_USER_DATA_DIR` 覆盖 userData 目录；也可用 `--file` / `MOCK_RULES_FILE` 显式指定文件。）

## 首选：用 CLI 操作（安全、幂等、有校验）

下面命令里的 `scripts/mock-rule.mjs` **随本 skill 一起分发**（就在本 skill 目录下的 `scripts/`），只依赖 node 内置模块，无需安装依赖。
由本 skill 触发时以 skill 目录为基准执行即可；若在 vjtools 源码仓库根目录手动操作，仓库根也有同名脚本，命令完全一致。
（脚本定位的是 vjtools 软件的配置文件，走绝对路径 `~/Library/Application Support/vjtools/…`，与脚本位置、当前工作目录无关。）
